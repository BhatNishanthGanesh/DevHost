...
const CategoryPage = () => {
  ...
  const fetchCategoryUrls = async (category: string) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://alumni-connect-backend-iydi.onrender.com/get-all-urls');
      ...
      const filteredUrls = data.data.filter((item: { category: string; url: string }) => item.category === category);
  ...
  // Function to handle category button click for navigation
  const handleCategoryClick = (category: string) => {
    router.push(`/admin/User/postals/${category}`); // Navigate to the category page dynamically
  };
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {categoryUrls.map((item, index) => (
              <div key={index} className="border rounded-lg p-4 shadow-lg hover:shadow-2xl transition-shadow">
                <div className="flex items-center space-x-3 mb-4">
                  {/* Category Icon */}
                  {category === "web/app" && <FaReact size={30} color="#61dafb" />}
                  {category === "electronics" && <FaTools size={30} color="#4caf50" />}
                  {category === "others" && <FaBriefcase size={30} color="#ff9800" />}
          ...
          <div className="text-center text-gray-500">No URLs available for this category.</div>
        )}
      </div>
    </div>
  );
